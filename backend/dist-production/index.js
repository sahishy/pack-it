...
var createChatResponse = /* @__PURE__ */ __name(async ({ env, uid, tripId, messageId, message: message2, executionContext, isAnonymous = false }) => {
  ...
  if (existingUserMessage && (existingUserMessage.userId !== uid || existingUserMessage.tripId !== tripId || existingUserMessage.role !== "user" || existingUserMessage.content !== message2)) {
  ...
  const input = savedMessages.slice(-12).filter(({ role, content }) => (role === "user" || role === "assistant") && content).map(({ role, content }) => ({ role, content }));
  const actions = [];
...
var createHandler = /* @__PURE__ */ __name(({
  ...
  try {
    ...
    if (pathname === "/v1/ai/chat") {
      const input2 = await readJson(request, chatSchema);
